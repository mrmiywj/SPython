...
;; ----------- B -----------  
  ;;Karan (*B4)
  (method (macaroni-time new-val) ;; how negate bool is gonna be called and implemented
    (set! val new-val)
    val)
;; ----------- B ----------- 
...
(define (negate-bool bool)  
  (let ((current-val (ask bool 'true?)))
    ; (cond ((current-val) (begin (ask bool 'macaroni-time #f) bool))
    ;   (else (begin (ask bool 'macaroni-time #t) bool)))))
    (if current-val
      (begin (ask bool 'macaroni-time #f)
        bool)
      (begin (ask bool 'macaroni-time #t)
        bool))))
; -------------------------------
...
;;----------------------------------------PERSON A--------------------------;;
  ...
  ;; (*A4) Return a boolean value #t or #f
  ;; depending on whether or not an item
  ;; is in the given list of inputs.
    ;;(let ((input-other (ask 'val)))
    (let ((input-other (ask self 'val)))
      (if (null? (filter (lambda (x) (ask x 'true?)) (map (lambda (x) (py-apply (ask x '__eq__) (list other))) input-other)))
      ;;*FALSE*
      ;;*TRUE*)))
      *PY-FALSE*
      *PY-TRUE*)))
