...
(define (handle-character line-obj env)
  (let ((token (ask line-obj 'next)))
    (if (colon? token)
      '()
      (cons token (handle-character line-obj env)))))
...
;;----------------------------------------PERSON A--------------------------;;
         ...
         ;;(*A5) Logical and for python
         ;; Follow the rules of a logical and
         ;; 1 and 0 = false; 0 and 0 = false
         ;; 0 and 1 = false; 1 and 1 = true
         ;;(logical-and token))
        (cond ((ask val 'true?) (py-eval line-obj env))
        (else ((begin (eat-tokens line-obj) *PY-FALSE*))
          )))
        ;;(logical-and token))
        ;;(define (logical-and token)
        ;;  (if (ask val 'true?) 
        ;;    (py-eval line-obj env)
        ;;    (begin (eat-tokens line-obj) *PY-FALSE*)))
	      ((or? token)
	       ;;(py-error "TodoError: Person A, Question 5"))
         ;;(*A5) Logical or for python
         ;; Follow the rules of a logical or
         ;; 1 or 0 = true; 0 or 0 = false
         ;; 0 or 1 = true; 1 or 1 = true
         ;;(logical-or token))
        (if (ask val 'true?)
          (begin 
            (eat-tokens line-obj) val)
          (py-eval line-obj 
            env)))
;;-------------------------------------------------------------------------;;
	      ;; test for membership
        ...
        ;;Karan (*B5)
          (let 
            ((following-list (eval-item line-obj env)))
            ;;(ask (following-list ((eval-item line-obj env))))
            ;;(lambda (following-list) (following-list (eval-item line-obj env)))
            (ask following-list '__contains__ val)))
        ((not? token)
        ;;Karan (*B5)
        (let ((token-check ;;going through the token 
              (ask line-obj 'next)))
          (cond (in? token-check) (let ((following-list (eval-item line-obj env)))
              (else (negate-bool (ask following-list '__contains__ val)))))))
...
(define (make-while-block line-obj env)
  ;;(define (while-block line-obj env)
  (let ((while-pred (list-the-while line-obj env))
    ...
    (while-body (read-block (spacing line-obj) env)))
  (list '*BLOCK* '*WHILE-BLOCK*  while-pred while-body)))

;;(* Common 6)
;;the MAKE-WHILE-BLOCK constructor, and the
...
;;(* Common 6)
  (let ((body (cadddr-block while-block)))
...
;;(* Common 6)
(define (list-the-while line-obj env)
  (let ((indentation (spacing line-obj))
    (pred-list (handle-character line-obj env)))
  (cons indentation pred-list)))

;;(* Common 6)
(define (eval-while-block block env)(let ((pred (while-block-pred block))
	(body 
    (while-block-body block))
	(else-clause 
    (while-block-else block)))
    (let 
...
(define (make-for-block line-obj env)
  ...
  (let 
    ((var (ask line-obj 'next)) (collection (begin (ask line-obj 'next)
      ;;(hunter-gatherer line-obj env)))
      (hunter-gatherer line-obj env))) (body (read-block (ask line-obj 'indentation) 
      env)))
    ;;))
...
;;----------------------------------------PERSON A--------------------------;;
;;(*A7)
;; Helper method for the for look constructor
;; Hunts for things after the colon and gathers them up
(define (hunter-gatherer line-obj env)
  (let ((token (ask line-obj 'next)))
    (cond ((colon? token) '())
    (else (cons token (hunter-gatherer line-obj env))))))
;;--------------------------------------------------------------------------;;

(define (cadddr-block block)
  (cadddr block))
